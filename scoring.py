...
def get_team_1():
    ...
    if games == []:
        raise ValueError('This team is not on the roster. Check your spelling! (Capitalization is not required.)')
...
def create_score_dict():
    ...
    score_1 = int(input(f"how many points will {user_team_1} end up with? >>> "))
    score_2 = int(input(f"how many points will {user_team_2} end up with? >>> "))
    if score_1 > score_2:
        winner = user_team_1
    elif score_1 < score_2:
        winner = user_team_2
    elif score_1 == score_2:
        team = input('Predicting a draw means predicting a penatlies round. Which team will win? >>> ')
